...
for i in range(3):
    ...
    l1, = axs[0].plot(range(0, epochs, strid), y[0][::strid], 'r' + line[i], linewidth=linewidth[i],)
    l2, = axs[0].plot(range(0, epochs, strid), y[1][::strid], 'b' + line[i], linewidth=linewidth[i])
    l3, = axs[0].plot(range(0, epochs, strid), y[2][::strid], 'g' + line[i], linewidth=linewidth[i])
    handles += l1, l2, l3
# axs[0].
...
#     linewidth = 1
#
#     l1, = axins.plot(range(0, epochs, strid), y[0][::strid], 'r' + line[i], linewidth=linewidth, )
#     l2, = axins.plot(range(0, epochs, strid), y[1][::strid], 'b' + line[i], linewidth=linewidth)
#     l3, = axins.plot(range(0, epochs, strid), y[2][::strid], 'g' + line[i], linewidth=linewidth)
#     ys += [y[0][::strid],
#            y[1][::strid],
#            y[2][::strid]]
# axins.grid(None)
...
#
# # 设置放大区间
# zone_left = int(10 / strid)
# zone_right = int(20 / strid)
#
# # 坐标轴的扩展比例（根据实际数据调整）
...
#
# # X轴的显示范围
# x = [i for i in range(0, epochs, strid)]
...
# plt.savefig("imgs/local_acc.pdf", bbox_inches='tight', dpi=100, pad_inches=0.0)
plt.show()